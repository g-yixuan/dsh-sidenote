...
> 名字未定稿前，文中一律用 `dsh-side-chat`；若改名，全局替换即可。
...
**标题**：`Show HN: dsh-side-chat – Codex-style side chat and selection annotations for DeepSeek Harness`
...
>
> dsh-side-chat is a plugin for DeepSeek Harness (built on dsh-better-sidebar's tab service) that does two things and ties them together:
>
...
>
> 所以我写了 dsh-side-chat（DSH 插件，基于 dsh-better-sidebar 的 Tab 服务）：
>
...
>
> 演示：[GIF/视频]
> 安装：`dsh plugin --profile web add dsh-side-chat`
> 仓库：[GitHub 链接]（MIT）
>
...
>
> dsh-side-chat 是 DSH（DeepSeek Harness）的插件，消费 dsh-better-sidebar 的侧边栏服务：
>
...
>
> 安装：`dsh plugin --profile web add dsh-side-chat`（需要先装 dsh-better-sidebar）
> 仓库：[GitHub 链接]
>
> 求试用求意见 🙏
...
>
> 起名废物，先叫 dsh-side-chat。开源 MIT，一行命令装：[链接]
>
> [效果图/视频]
