#!/usr/bin/env bash
# =============================================================================
# dsh-side-chat 挂载冒烟编排（仿 dsh-better-sidebar scripts/e2e-mount.sh）：
#
...
if [ -z "$TARBALL" ]; then
  TARBALL="$(ls "$ROOT"/dsh-side-chat-*.tgz 2>/dev/null | head -1 || true)"
fi
[ -n "$TARBALL" ] && [ -f "$TARBALL" ] || die "找不到 tarball——先运行 pnpm build && pnpm pack"
TARBALL="$(cd "$(dirname "$TARBALL")" && pwd)/$(basename "$TARBALL")"
say "tarball: $TARBALL"

SCRATCH="${DSH_HOME_BASE:-$(mktemp -d /tmp/dsh-side-chat-e2e.XXXXXX)}"
...
node -e '
  ...
  const missing = ["dsh-better-sidebar", "dsh-side-chat"].filter((b) => !bundles.includes(b));
  if (missing.length) { console.error("挂载未注册:", missing.join(", ")); process.exit(1); }
' "$PROFILE_DIR/package.json"
say "挂载已注册：dsh-better-sidebar + dsh-side-chat"
...
say "通过：dsh-side-chat 挂载到真实 DSH 后无头渲染未崩溃"
